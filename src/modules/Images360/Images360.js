...
export class Images360 extends EventDispatcher {
	...
	focus(image360) {
		viewer.initialEDLState = viewer.getEDLEnabled();

		if (viewer.initialEDLState == false) {
			viewer.setEDLEnabled(true); // Gal
		}

		viewer.setEDLOpacity(0.0); // Nejc

		if (this.focusedImage !== null) {
			this.unfocus();
		}

		// GAL -> Da vemo, kdaj vstopimo v street view
		this.viewer.dispatchEvent({
			type: '360image_focused', detail: { image: image360 }
		});
	...
	unfocus() {
		this.elUnfocus.style.display = "none";
		viewer.setEDLEnabled(viewer.initialEDLState); // Gal
		this.viewer.setEDLOpacity(1); // Nejc
		...
		this.viewer.dispatchEvent({
			type: '360image_unfocused', detail: { image: this.focusedImage }
		});
